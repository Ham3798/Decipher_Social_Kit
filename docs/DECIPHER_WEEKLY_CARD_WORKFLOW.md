...
- Run `node skills/decipher-weekly-cards/scripts/audit-skill-privacy.mjs` before staging reusable skill changes.
...
Resolve conflicts in this order: latest user correction, current request notes and photos, reusable skill conventions, then prior-week outputs as an optional sanity check. Exact names, cohort, title, presenter order, date, and platform handles must follow the latest instruction.
...
- Preserve manual title newlines and use only a few bounded whole-block font tiers for dense answers.
...
5. Preserve interview answers by default, correcting only spelling, spacing, and light sentence flow. Use one question and complete answer per detail card.
6. Render cards through the existing React template and export PNGs.
7. Visually inspect every card and verify dimensions with `sips`.
...
- A no-speaker week still receives the established blank date-only speaker card.
- Interview output contains one detail card per supplied question; do not force five slides when the source contains a different count.
- If text is dense, improve paragraphs and title breaks before using restrained font fitting. Do not silently delete content.
...
- Every requested name, cohort, order, date, title, and handle matches the latest instruction.
